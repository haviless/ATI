...
Procedure TFManComTipo.FormCreate(Sender: TObject);
Begin
   xSQL := ' select TIPOID, nombre from '+DMATI.wwEsquemaATI+'ATI_TIPOC '
      + ' order by TO_NUMBER(TIPOID)';

   DMATI.cdsQry10.Close;
   DMATI.cdsQry10.DataRequest(xSQL);
   DMATI.cdsQry10.open;
   dbgTecnologia.DataSource := DMATI.dsQry10;
...
Procedure TFManComTipo.wwDBGrid1IButtonClick(Sender: TObject);
Begin
   ...
   xSQL := 'select MAX(TO_NUMBER(TIPOID)+1) NUMERO from '+DMATI.wwEsquemaATI+'ATI_TIPOC';
   DMATI.cdsQRY.Close;
   DMATI.cdsQRY.DataRequest(xSQL);
   DMATI.cdsQRY.Open;
   DMATI.cdsQry10.Append;
   DMATI.cdsQry10.FieldByName('TIPOID').AsString := IntToStr(DMATI.cdsQry.FieldByName('NUMERO').AsInteger);
   dbeNombre.SetFocus;

End;

Procedure TFManComTipo.BModificarClick(Sender: TObject);
Begin
   // verifica que nombre de equipo no este vacío
   If length(DMATI.cdsQry10.FieldByName('NOMBRE').AsString) = 0 Then
      Begin
   ...
   xSQL := ' select NOMBRE '
      + ' from '+DMATI.wwEsquemaATI+'ATI_TIPOC '
      + ' where NOMBRE=' + quotedstr(DMATI.cdsQry10.FieldByName('nombre').AsString)
      + ' and TIPOID<>' + inttostr(DMATI.cdsQry10.FieldByName('TIPOID').AsInteger);
   DMATI.cdsQry.Close;
   DMATI.cdsQry.DataRequest(xSQL);
   DMATI.cdsQry.Open;
   ...
   If MessageDlg(' ¿Grabar Registro? ', mtConfirmation, [mbYes, mbNo], 0) = mrYes Then
      Begin
         If xModo = 'I' Then
            Begin
               xSQL := ' Insert into '+DMATI.wwEsquemaATI+'ATI_TIPOC(NOMBRE, TIPOID) '
                  + ' values( '
                  + quotedstr(DMATI.cdsQry10.FieldByName('NOMBRE').AsString) + ','
                  + quotedstr(DMATI.cdsQry10.FieldByName('TIPOID').AsString) + ')';
            End
               //Modifica la Oficina
         Else
            Begin
               xSQL := ' update '+DMATI.wwEsquemaATI+'ATI_TIPOC '
                  + ' set NOMBRE =' + QuotedStr(DMATI.cdsQry10.FieldByName('NOMBRE').AsString)
                  + ' where TIPOID =' + QuotedStr(DMATI.cdsQry10.FieldByName('TIPOID').AsString);
            End;
...
Procedure TFManComTipo.Z2bbtnCanc3Click(Sender: TObject);
Begin
   DMATI.cdsQry10.CancelUpdates;
   DMATI.cdsQry10.open;
   DMATI.cdsQry10.Refresh;
   dbgTecnologia.Enabled := true;
   dbgRel.Enabled := true;
   pnlPie.Enabled := true;
   pnlActualizacion.Visible := False;
...
Procedure TFManComTipo.dbgTecnologiaDblClick(Sender: TObject);
Begin
...
Procedure TFManComTipo.dbgTecnologiaKeyDown(Sender: TObject;
   ...
   If (key = VK_Delete) And (ssCtrl In Shift) Then
      Begin
         ...
         If MessageDlg(' ¿Eliminar Registro? ', mtConfirmation, [mbYes, mbNo], 0) = mrYes Then
            Begin
               xSQL := 'delete from '+DMATI.wwEsquemaATI+'ATI_TIPOC '
                  + 'where TIPOID=' + IntToStr(DMATI.cdsQry10.FieldByName('TIPOID').AsInteger);
               Try
                  DMATI.ATISkt.AppServer.EjecutaSQL(xSQL);
               Except
                  ...
                  exit;
               End;
               DMATI.cdsQry10.Delete;
            End;
      End;
End;

Procedure TFManComTipo.dbgTecnologiaRowChanged(Sender: TObject);
Begin
   xSQL := 'SELECT DISTINCT  '
      + ' ATI_NOM_PAR_EQU.NOMBRE, '
      + ' ATI_NOM_PAR_EQU.NOMBRE_PARTE_EQUIPOID,  '
      + ' ATI_TIPOC.TIPOID  '
      + ' FROM '+DMATI.wwEsquemaATI+'ATI_TIPOC, '+DMATI.wwEsquemaATI+'ATI_TIP_CTRL, '+DMATI.wwEsquemaATI+'ATI_NOM_PAR_EQU  '
      + ' WHERE ATI_TIP_CTRL.NOMBRE_PARTE_EQUIPOID = ATI_NOM_PAR_EQU.NOMBRE_PARTE_EQUIPOID '
      + ' AND ATI_TIP_CTRL.TIPOID = ATI_TIPOC.TIPOID  '
      + ' AND ATI_TIP_CTRL.TIPOID =' + QuotedStr(DMATI.cdsQry10.FieldByName('TIPOID').AsString);
...
Procedure TFManComTipo.wwIButton1Click(Sender: TObject);
Begin
   ...
   DMATI.cdsQryR.FieldByName('TIPOID').AsInteger := DMATI.cdsQry10.FieldByName('TIPOID').AsInteger;
...
Procedure TFManComTipo.cargacombo;
Begin
...
Procedure TFManComTipo.BitBtn3Click(Sender: TObject);
Begin
...
Procedure TFManComTipo.dblcEquipoChange(Sender: TObject);
Begin
...
Procedure TFManComTipo.BitBtn4Click(Sender: TObject);
Begin
   // verifica que nombre de equipo no este vacío
   ...
   xSQL := ' select NOMBRE_PARTE_EQUIPOID '
      ...
      + ' and TIPOID=' + QuotedStr(DMATI.cdsQry10.FieldByName('TIPOID').AsString);
   DMATI.cdsQry.Close;
   DMATI.cdsQry.DataRequest(xSQL);
   DMATI.cdsQry.Open;
   If DMATI.cdsQry.Recordcount > 0 Then
   ...
   If MessageDlg(' ¿Grabar Registro? ', mtConfirmation, [mbYes, mbNo], 0) = mrYes Then
      Begin
         If xModo = 'I' Then
            Begin
               xSQL := ' insert into '+DMATI.wwEsquemaATI+'ATI_TIP_CTRL (NOMBRE_PARTE_EQUIPOID, TIPOID) '
                  + ' values ('
...
Procedure TFManComTipo.dbgRelKeyDown(Sender: TObject; Var Key: Word;
   Shift: TShiftState);
Begin
   //Captura la centetencia Ctrl+Del para eliminar
   If (key = VK_Delete) And (ssCtrl In Shift) Then
         ...
         If MessageDlg(' ¿Eliminar Registro? ', mtConfirmation, [mbYes, mbNo], 0) = mrYes Then
            Begin
               xSQL := 'delete from '+DMATI.wwEsquemaATI+'ATI_TIP_CTRL '
                  ...
                  + 'AND TIPOID=' + QuotedStr(DMATI.cdsQryR.FieldByName('TIPOID').AsString);
               Try
                  DMATI.ATISkt.AppServer.EjecutaSQL(xSQL);
               Except
                  ShowMessage('No se pudo Eliminar');
...
Procedure TFManComTipo.dbgRelDblClick(Sender: TObject);
Begin
...
Procedure TFManComTipo.FormClose(Sender: TObject;
...
Procedure TFManComTipo.edtBuscarChange(Sender: TObject);
Begin
   DMATI.cdsQry10.Locate('nombre', VarArrayOf([TRIM(edtBuscar.Text)]), [loPartialKey]);
End;

procedure TFManComTipo.edtBuscarKeyPress(Sender: TObject; var Key: Char);
begin
If Key = #13 Then
    Begin
